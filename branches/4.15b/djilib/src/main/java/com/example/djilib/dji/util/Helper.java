...
import com.example.djilib.dji.healthinfomation.HealthInfoMatchSDKError;
...
public class Helper {
    ...
    private static List<HealthInfo> hmsJson;

    public static List<HealthInfo> getHmsInfo(Context context) {
        if (hmsJson == null) {
            synchronized (Helper.class) {
                if (hmsJson == null) {
                    // the newest json file , can find by github
                    hmsJson = getObjFromJsonFile(context, "hms.json", new TypeToken<List<HealthInfo>>() {
                    }.getType());
                }
            }
        }
    ...
    private static List<HealthInfoMatchSDKError> hmsMatchSDKError;

    public static List<HealthInfoMatchSDKError> getHmsMatchSDKError(Context context) {
        if (hmsMatchSDKError == null) {
            synchronized (Helper.class){
                if (hmsMatchSDKError == null) {
                    // the newest json file , can find by github
                    hmsMatchSDKError = getObjFromJsonFile(context, "hms_match_sdkerror.json", new TypeToken<List<HealthInfoMatchSDKError>>() {
                    }.getType());
                }
            }
        }
